...
const ListLines = ({data, variant}) => {
  ...
  useEffect(() => {
    setContent(<>
      {data.map((line) => {
        // console.log(line)
        return (<Card key={line.line_id} title={`Linha`}
                      badge={(<div className="d-flex flex-wrap gap-1">
                        <Badge
  ...
  return (<div style={{marginTop: '1rem'}}>
    {variant === "similar-lines" ? (<ScrollX>{content}</ScrollX>) : <Grid variant={variant}>{content}</Grid>}
  </div>);
}
