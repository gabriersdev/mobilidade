const news = [
  {
    title: "Linha 4992 Sabará ⇄ Terminal São Gabriel deixa de operar nesta segunda (02/06)",
    resume: "Os horários foram transferidos para a linha 4991. Veja o que muda para os passageiros.",
    img: "",
    publishDate: "2025-06-02T00:00:00.000-03:00",
    content: [
      "Os passageiros que usavam a linha 4992, Sabará ⇄ Terminal São Gabriel Via Av. José Cândido da Silveira acordaram na manhã dessa segunda (02/05) com a notícia que a linha havia sido suspensa",
      "A linha operava somente durante a semana, desde 2021 e existia desde 2016. As viagens partiam do bairro Siderúrgica, no centro da cidade, passando pela Av. José Cândido da Silveira, depois pela Av. Cristiano Machado e o Terminal São Gabriel e voltava, terminando a viagem no Terminal Rodoviário de Sabará.",
      "O motivo da desativação da linha não foi oficializado, mas provavelmente ocorreu pela baixa adesão do público, já que boa parte do percurso também é feito pelas outras linhas da Viação Cuiabá.",
      "<>Os horários de partida foram transferidos para a linha 4991 - Sabará ⇄ Terminal São Gabriel Via Barraguinha. Durante os dias úteis, os horários que antes eram da linha 4992 foram transferidos para a 4991, que agora opera entre as 04h55 e 00h em dias úteis. <Link to={\"/lines/18#partidas\"}>Confira o quadro de horários atualizado da linha 4991.</Link></>"
    ],
  },
  {
    title: "Fique ligado: vem aí o Aniversário de Sabará",
    resume: "Durante os dias 15 à 17 de julho, eventos ocorrem na cidade em comemoração ao Aniversário de Sabará",
    img: "",
    publishDate: "2025-06-29T00:00:00.000-03:00",
    content: [
      "Em comemoração ao aniversário da cidade, todas as linhas operadas pela Vinscol e as linhas 4970, 4986, 4987 e 4991, operadas pela Viação Cuiabá têm o itinerário alterado, deixando de passar pelo centro histórico e Avenida Perimetral e parte da Avenida Prefeito Victor Fantini, passando pela Avenida Central.",
      "Para maiores informações fique ligado nos avisos na página das linhas. Sempre avisaremos quando uma mudança ocorrer."
    ],
  },
  {
    title: "Calendário do Aniversário de Sabará 2025",
    resume: "Confira o calendário dos eventos do Aniversário de Sabará 2025",
    img: "",
    publishDate: "2025-07-05T00:00:00.000-03:00",
    content: [
      "Durante as datas listadas abaixo os ônibus operados pela Vinscol não passarão pelo Centro Histórico. Está previsto para que, entre os dias 12 e 20 de julho, todas as linha da Vinscol e as linhas 4986, 4987, 4988 e 4991 operadas pela viação Cuiabá não passarão pela Av. Perimetral e parte da Av. Prefeito Victor Fantini, sendo parte do itinerário transferido para a Av. Central.",
      "Sexta-feira - 4 de julho: 18h - Abertura Oficial (Praça Getúlio Vargas), 20h - Primeiro Arraiá de Sabará",
      "Sábado - 5 de julho: 18h - Abertura Oficial, 18h30 - DJ Jan Paul, 19h - Edson Rodrigues, 21h30 - Alan e Alex",
      "Sábado - 12 de julho: 10h - Rap de Quebrada Cultural (Pista de Skate Matheus Todynho - Rua José Brochado Gomes, 296 - Vila Santo Antônio de Pádua), 18h30 - DJ Wanderson, 19h - Monique Ellen, 21h30 - Gabi Martins",
      "Quarta-feira - 16 de julho: 18h - Abertura Oficial, 19h - Quero Mais, 21h30 - Mumuzinho",
      "Quinta-feira - 17 de julho: 18h - Abertura Oficial, 20h - Zé Márcio, 22h30 - Os Caras do Arrocha",
      "Sexta-feira - 18 de julho: 18h - Abertura Oficial, 19h - Vicente Dendem, 21h30 - Bruna Lipiani",
      "Sábado - 19 de julho: 18h - Abertura Oficial, 19h - Sapotchen, 21h30 - Tianastácia",
      "Domingo - 20 de julho: 9h - Encontro de Bandas (Praça Santa Rita, Centro), 18h - DJ Wanderson, 19h - Caminhos 2000",
      "Segunda-feira - 21 de julho: 18h - Abertura Oficial, 19h - Yeshua, 20h - Banda Ágape"
    ],
  },
  {
    title: "Nova linha 4990 - Adelmolândia e Paciência para Belo Horizonte",
    resume: "A linha começa a operar hoje (14/07/2025) e atende os passageiros desses bairros.",
    img: "/news/neobus-44089-ob.jpg",
    publishDate: "2025-07-14T00:00:00.000-03:00",
    content: [
      "Uma nova linha foi criada para atender os passageiros que moram nos bairros Paciência e Adelmolândia. A linha 4990 passou a operar na manhã dessa segunda-feira (14/07).",
      "Inicialmente com 4 partidas em dias úteis (de segunda à sexta). A nova linha será uma mão na roda para os passageiros que dependiam de fazer baldeação com as linhas 04 e 03 operadas pela Vinscol, cujo os horários foram abruptamente reduzidos desde o início da pandemia de Covid-19 e nunca mais foi reestabelecido.",
      "<>Veja as <Link to={\"/lines/42\"}>informações atualizadas e em detalhes da linha 4990</Link>.</>"
    ],
  },
  {
    title: "Feriado de 07 de setembro - mudanças de itinerário",
    resume: "Durante o feriado de 07 de setembro (domingo) todas as linhas operadas pela Vinscol não passarão pelo centro Histórico.",
    img: "/news/neobus-44101-ob.jpg",
    publishDate: "2025-08-30T00:00:00.000-03:00",
    content: [
      "No domingo, feriado de 07 de setembro todas as linhas operadas pela Vinscol não passarão pelo centro Histórico, em razão das comemorações e desfiles no Centro, seguindo viagem pela Av. Prefeito Victor Fantini e Av. Perimetral, sem entrar na Praça da Getúlio Vargas (Matriz), durante todo o dia, no sentido ida e volta.",
      "<>As linhas operadas pelo Consórcio Metropolitano <Link to={\"/company/4\"}>(Transporte Coletivo Metropolitano - MG)</Link> não são afetadas pelas comemorações de 7 de setembro.</>",
    ],
  },
  {
    title: "Mudanças nos quadros de horários das linhas nos feriados do Natal e Ano Novo",
    resume: "Durante as vésperas (24 e 31) e os feriados (25/12 e 01/01), os quadros de horários das linhas mudam.",
    img: "/news/neobus-44115.jpg",
    publishDate: "2025-11-30T00:00:00.000-03:00",
    content: [
      "<>Nesses dias, geralmente até as 12 ou 14 horas, as linhas operam no quadro de horários do dia corrente e depois no quadro de horários de feriado (geralmente o mesmo de domingo). Para evitar qualquer transtorno causado por informações incorretas, optamos, nas vésperas de feriados, definir todas as linhas com o horário de partida de domingo e/ou feriados. Nos dias (25 e 01/01) os horários de partidas para todas as linhas também serão de domingo e/ou feriados. <Link to={\"/lines\"}>Consulte os horários das linhas aqui.</Link>",
    ],
  },
  {
    title: "Aumento escorchante para o sistema metropolitano de 8.93%",
    resume: "Governo do estado autorizou reajuste que começa a valer no dia 09/01/2026. O reajuste superou qualquer um dos reajustes entre 2021 e 2025. A média em reais para as linhas de Sabará foi de R$ 0,80 centavos.",
    img: "/news/neobus-44069.jpg",
    publishDate: "2026-01-06T00:00:00.000-03:00",
    content: [
      "11 meses depois de começar a distribuir 850 ônibus para serem usados pelas companhias da RMBH, o Governo do estado de Minas Gerais autorizou um reajuste escorchante para as linhas do sistema metropolitano: intermunicipais, metropolitanos bairros, MOVE metropolitano, executivos e executivos rodoviários.",
      "<>As linhas de Sabará, aumentaram, em média R$ 0,80 centavos, com destaque para a linha <Link to={\"/news/6\"}>4986 - Sabará -> Belo Horizonte | Bandeirante</Link>, em que a passagem passará dos R$ 10,80 para R$ 11,75. São R$ 0,90 a mais sem nenhuma promessa de melhoria. Algumas outras cidades tiveram uma média de reajuste menor. O ano de 2025 fechou com inflação </>",
      "<>A tarifa mais cara do sistema, da linha 3213 - Betim -> Aeroporto Internacional Tancredo Neves via Aeroporto da Pampulha passará de R$ 75,70 para R$ 82,45. O reajuste foi anunciado nesta segunda-feira (05) e começará a valer a partir da próxima sexta-feira, 09/01. Créditos adquiridos até o dia 08/01 serão debitados conforme tarifa de 2025, inclusive se usados após o dia 09/01 e tem validade até a expiração ou uso.</>",
      "<>Confira a parte do documento sobre os <Link to={\"https://github.com/gabriersdev/mobilidade/blob/main/public/docs/doc-aumento-metropolitano-2026.pdf\"}>reajustes das linhas de Sabará.</Link></>",
    ],
  },
  {
    title: "Horários de ônibus no Carnaval 2026",
    resume: "Confira as alterações nos quadros de horários das linhas municipais e intermunicipais durante o feriado de Carnaval.",
    img: "/news/carnaval.jpg",
    publishDate: "2026-02-13T00:00:00.000-03:00",
    content: [
      "Durante o feriado de Carnaval, o transporte coletivo terá alterações em seus quadros de horários. A medida visa adequar a oferta de viagens à demanda de passageiros, que costuma reduzir neste período.",
      "Na segunda-feira (16/02) e terça-feira (17/02), as linhas operarão com horário de domingo/feriado. Na quarta-feira de cinzas (18/02), o funcionamento será com horário atípico ou de domingo até as 12h, retornando ao normal após esse horário.",
      "Mudanças de itinerário também ocorrerão. As linhas municipais que passam pelo Centro de Sabará e Av. Prefeito Victor Fantini passarão pela Av. Central até a entrada para o bairro Córrego da Ilha. As linhas metropolitanas que passam pelo Centro de Sabará e Av. Prefeito Victor Fantini também passarão pela Av. Central. As linhas de bairros também podem sofrer alterações no itinerário.",
      "Todas as linhas metropolitanas que passam pelo Centro de Belo Horizonte, assim como nos arredores passarão apenas pela Av. Andradas e Av. Contorno. Nesse caso, consulte o motorista antes de embarcar ou entre em contato com a empresa responsável pela linha."
    ],
  },
]

const returnNews = news.map((n, i) => {
  return {...n, ["id"]: i}
});

export default returnNews;
