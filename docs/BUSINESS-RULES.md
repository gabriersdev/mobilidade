...
- **Exibição de Previsões (Rastreador em Tempo Real):** No modo de acompanhamento ao vivo, a interface deve reagir a atualizações de previsões vindas passivamente do servidor via WebSocket. Sempre que um novo ponto é selecionado, o cliente **deve cancelar a inscrição** (`unsubscribe-predictions`) do ponto anterior e **inscrever-se** (`subscribe-predictions`) na nova "sala" do ponto pelo Socket, sem gerar polling ou overhead de requisições manuais ao servidor. A exibição de próximos horários (informações adicionais) varia conforme o estado do veículo. Se o horário corrente for de **Partida/Saída** (indicado por `order_departure_point == 1`), a interface exibe *apenas* as próximas previsões de partida. Se for de **Aproximação** (demais pontos da rota), a interface exibe *apenas* as próximas previsões de aproximação.
